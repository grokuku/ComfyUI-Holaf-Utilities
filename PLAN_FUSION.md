...
| 2 | Refactorer le `__init__.py` AIH (~1812 lignes) : suppression du chargement via `sys.modules`, imports standardisés, préfixe `/aih/*` conservé | À faire (chantier C) |
...
| 2 | Intégrer le mode miroir local complet (`store.py`, `sync_engine`, `embedding_engine`, `local_source`, routes `/aih/local/*`) | 🟠 Socle terminé (chantier A : package `aih/` + bootstrap sys.path — commits 25a921d store / c6ec494 sync_engine / 474e006 embedding_engine / ad20a71 local_source, validés hors-ComfyUI par harnais 53/53) ; routes `/aih/local/*` restent au chantier B |
